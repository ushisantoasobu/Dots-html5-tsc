    ...
    MainCtrl.setInitialSetting = function (canvas) {
        ...
        for (var i = 0; i < MainCtrl.ROW_COUNT; i++) {
            for (var j = 0; j < MainCtrl.COLUMN_COUNT; j++) {
                ...
                var dot = new dots.Dot(MainCtrl.DOTS_DISTANCE * (j + 1), MainCtrl.DOTS_DISTANCE * (i + 1), 20, color);
    ...
    */
    MainCtrl._canvasTouchStartHandler = function (e) {
        ...
        for (var i = len - 1; i >= 0; i--) {
            ...
            if (MainCtrl.touchStartPoint.x > dot.x - MainCtrl.TOUCH_RANGE && MainCtrl.touchStartPoint.x < dot.x + MainCtrl.TOUCH_RANGE && MainCtrl.touchStartPoint.y > dot.y - MainCtrl.TOUCH_RANGE && MainCtrl.touchStartPoint.y < dot.y + MainCtrl.TOUCH_RANGE) {
    ...
    */
    MainCtrl._canvasTouchMoveHandler = function (e) {
        ...
        for (var i = len - 1; i >= 0; i--) {
            ...
            //いずれかのドットに触れたとき
            if (x > dot.x - MainCtrl.TOUCH_RANGE && x < dot.x + MainCtrl.TOUCH_RANGE && y > dot.y - MainCtrl.TOUCH_RANGE && y < dot.y + MainCtrl.TOUCH_RANGE) {
                    ...
                    if (MainCtrl.currentTargetColor === dot.colorId) {
                        ...
                        if (dot.selected === false) {
                            ...
                            if (dot.positionIndex === lastIndex - 1 && MainCtrl._checkExistDotLeft(lastIndex)) {
                                ...
                                var line = new dots.Line(lastDot.x, lastDot.y, dot.x, dot.y);
        ...
        var line = new dots.Line(MainCtrl.touchStartPoint.x, MainCtrl.touchStartPoint.y, e.changedTouches[0].screenX - MainCtrl.stage.getCanvas().offsetLeft, e.changedTouches[0].screenY - MainCtrl.stage.getCanvas().offsetTop);
    ...
    MainCtrl._canvasTouchEndHandler = function (e) {
        ...
        if (MainCtrl.selectedDotsArray.length > 1) {
            ...
            for (var i = MainCtrl.dotContainer.getNumChildren() - 1; i >= 0; i--) {
                ...
                dot.startAnimation(dot.x, dot.y, dot.x, dot.y + (dot.positionIndex - previousIndex) / MainCtrl.COLUMN_COUNT * MainCtrl.DOTS_DISTANCE, MainCtrl.FPS, 200 * ((dot.positionIndex - previousIndex) / MainCtrl.COLUMN_COUNT), function () {
                    console.log("test");
                });
            }
            ...
            for (var i = 0; i < newIndexArray.length; i++) {
                color = parseInt(Math.random() * dots.Dot.COLORLIST.length);
                var dot = new dots.Dot(MainCtrl.DOTS_DISTANCE * (newIndexArray[i] % MainCtrl.COLUMN_COUNT + 1), MainCtrl.DOTS_DISTANCE * (Math.floor(newIndexArray[i] / MainCtrl.COLUMN_COUNT) + 1), 20, color);
                dot.positionIndex = newIndexArray[i];

                // columunIndex = dot.positionIndex % MainCtrl.COLUMN_COUNT;
                MainCtrl.dotContainer.addChild(dot);
                dot.startAnimation(dot.x, dot.y - MainCtrl._getDeletedCountForColumn(dot.positionIndex, removedIndexArray) * MainCtrl.DOTS_DISTANCE, dot.x, dot.y, MainCtrl.FPS, 200 * MainCtrl._getDeletedCountForColumn(dot.positionIndex, removedIndexArray), function () {
                    console.log("test");
                });
            }
    ...
    */
    ...
    MainCtrl._updateIndex = function (index, removedIndexArray) {
        ...
        for (var i = 0; i < removedIndexArray.length; i++) {
            ...
            if ((index % MainCtrl.COLUMN_COUNT === removedIndexArray[i] % MainCtrl.COLUMN_COUNT) && index < removedIndexArray[i]) {
                count++;
            }
        }
